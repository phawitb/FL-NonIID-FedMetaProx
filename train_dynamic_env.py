...
def train_federated(model, df_data, algorithm, num_rounds=10, epochs=10, lr=0.005, mu=0.01, meta_lr=0.001, clusters=3):
    ...
    for round_num in range(num_rounds):
        ...
        if algorithm == "FedAdaptProx":  # Enhanced FedProx with Adaptive Weighting and KMeans Clustering
            # Perform KMeans clustering on client data
            X_combined = np.vstack([X_train_tensor.numpy() for _, dataset in client_data.items()])
            kmeans = KMeans(n_clusters=clusters, random_state=0).fit(X_combined)
            cluster_assignments = kmeans.labels_
...
for data_set in DATA_SET:
    for ds in ['noniid','normal']:
        ...
        # ---------------------------------
        algorithms = ["FedAvg","FedNova","FedProx","FedAdaptProx"]
        ...
        for algo in algorithms:
            ...
            df = pd.DataFrame([H])
            file_path = 'datas/trainning_dynamic_historys2.csv'
